...
public class Schedule implements Serializable {
    ...
    public List<ScheduleEntry> getEntriesForRoom(String room) {
        List<ScheduleEntry> result = new ArrayList<>();
        for (ScheduleEntry entry : entries) {
            if (entry.getRoom().equalsIgnoreCase(room)) {
                result.add(entry);
            }
        }
        return result;
    ...
    public static class ScheduleEntry implements Serializable, Comparable<ScheduleEntry> {
        ...
        /**
         * Returns the instructor name or "TBA" if none is assigned.
         *
         * @param lesson the lesson
         * @return instructor name or "TBA"
         */
        private String getInstructorName(Lesson lesson) {
            if (lesson.getInstructor() != null) {
                return lesson.getInstructor().getName();
            }
            return "TBA";
        }
    }
